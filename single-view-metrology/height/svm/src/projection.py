...
###  Z  ###
    ...
    cv2.line(img,(X1_e1[0],X1_e1[1]),Vx[:-1].astype(int),(0,0,255),1)     #blue
    cv2.line(img,(X2_e1[0],X2_e1[1]),Vx[:-1].astype(int),(0,0,255),1)
    cv2.line(img,(X3_e1[0],X3_e1[1]),Vx[:-1].astype(int),(0,0,255),1)

    cv2.line(img,(Y1_e1[0],Y1_e1[1]),Vy[:-1].astype(int),(0,255,0),1)     #green
    cv2.line(img,(Y2_e1[0],Y2_e1[1]),Vy[:-1].astype(int),(0,255,0),1)
    cv2.line(img,(Y3_e1[0],Y3_e1[1]),Vy[:-1].astype(int),(0,255,0),1)

    cv2.line(img,(Z1_e1[0],Z1_e1[1]),Vz[:-1].astype(int),(250,0,0),1)     #red
    cv2.line(img,(Z2_e1[0],Z2_e1[1]),Vz[:-1].astype(int),(250,0,0),1)
    cv2.line(img,(Z3_e1[0],Z3_e1[1]),Vz[:-1].astype(int),(255,0,0),1)


    cv2.line(img,Vx[:-1].astype(int),Vy[:-1].astype(int),(0,255,255),3)
    cv2.line(img,Vx[:-1].astype(int),Vz[:-1].astype(int),(0,255,255),3)
    cv2.line(img,Vy[:-1].astype(int),Vz[:-1].astype(int),(0,255,255),3)
    ...
    Hyz[0,2] = Hyz[0,2] + offset_x
    Hyz[1,2] = Hyz[1,2] + offset_x

    Hzx[0,2] = Hzx[0,2] - offset_y
    Hzx[1,2] = Hzx[1,2] + offset_y


    r,c,temp = img.shape
    #  img_padding = np.zeros((r*2,c*2,3),dtype=np.uint8)
    #  start = np.array([int(r/2),int(c/2)])
    #  end = start+img.shape[:2]
    #  img_padding[start[0]:end[0], start[1]:end[1]] = img
